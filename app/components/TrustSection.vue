<template>
  <section id="trust" class="py-20 px-6 bg-primary/5 border-y border-primary/5">
    <div class="max-w-5xl mx-auto">
      <div class="text-center mb-14 reveal">
        <p class="text-xs font-semibold text-primary uppercase tracking-widest mb-3">Kredibilitas</p>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-start">
        ...
        <!-- Kanan: Card kredibilitas -->
        <div class="grid grid-cols-1 gap-4">
          <div
            v-for="(point, index) in trustPoints"
            :key="index"
            class="reveal flex gap-4 items-start p-5 rounded-xl border border-primary/10 hover:border-primary/20 transition-colors"
          ...
          >
            ...
            <p class="text-sm text-text/80 leading-relaxed">{{ point.text }}</p>
          </div>
        </div>
      </div>

      <!-- Testimonials -->
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-16 reveal">
        <div
          v-for="(item, index) in testimonials"
          :key="index"
          class="p-6 rounded-2xl border border-primary/10 bg-white flex flex-col shadow-sm"
          :class="`reveal-delay-${index + 1}`"
        >
          <p class="text-text/80 text-sm leading-relaxed flex-1 mb-5">"{{ item.quote }}"</p>
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="font-semibold text-text text-sm">{{ item.name }}</p>
              <p class="text-xs text-text/60">{{ item.role }}</p>
            </div>
          </div>
        </div>
      </div>
...
const testimonials = [
  {
    quote: 'Panduan ini bantu saya sistematis pakai AI untuk coding. Dari chaos jadi punya framework jelas.',
    name: 'Budi',
    role: 'Software Developer',
    initial: 'B'
  },
  {
    quote: 'Akhirnya ada yang nulis dari pengalaman nyata, bukan sekadar teori. Langsung bisa dipraktikkan.',
    name: 'Siti',
    role: 'Fullstack Developer',
    initial: 'S'
  },
  {
    quote: 'Worth it. Saya pakai pendekatan dari sini di tim dan workflow jadi lebih rapi.',
    name: 'Andi',
    role: 'Tech Lead',
    initial: 'A'
  }
...
const trustPoints = [
  {
    text: 'Ditulis oleh praktisi yang sehari-hari bekerja dengan AI Coding Agent',
    icon: 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z'
  },
  {
    text: 'Berdasarkan pengalaman membangun aplikasi real — bukan proyek tutorial',
    icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z'
  },
  {
    text: 'Pendekatan yang mengikuti SDLC yang diadaptasi untuk era AI',
    icon: 'M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15'
  },
  {
    text: 'Tidak menjual hype — fokus pada strategi yang teruji dan realistis',
